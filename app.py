...
.badge {
    ...
    background: linear-gradient(135deg,#c9960c18,#f5c51818);
    border: 1px solid #c9960c33;
...
.hero h1 {
    ...
    background: linear-gradient(135deg,#fff8e7 40%,#f5c518);
...
.card:hover { border-color: #c9960c44; }
.card-title {
...
.step-num {
    ...
    color: #0a0800; border-radius: 50%; width: 26px; height: 26px;
...
.stButton > button {
    background: linear-gradient(135deg,var(--accent),var(--accent2)) !important;
    color: #0a0800 !important; border: none !important; border-radius: 12px !important;
...
.plan:hover { border-color: var(--accent); transform: translateY(-3px); box-shadow: 0 10px 28px #c9960c1a; }
.plan.hot { border-color: var(--accent); background: linear-gradient(135deg,#c9960c0d,var(--card)); }
.hot-badge {
    position: absolute; top: -11px; left: 50%; transform: translateX(-50%);
    background: linear-gradient(135deg,var(--accent),var(--accent2));
    color: #0a0800; padding: 2px 12px; border-radius: 100px;
...
.plan-price { font-family:'Syne',sans-serif; font-weight:800; font-size:1.8rem; color:var(--accent2); }
...
.plan-feat li::before { content:"✓"; color:var(--accent2); font-weight:700; }
.pay-btn {
    display:block; background:linear-gradient(135deg,var(--accent),var(--accent2));
    color:#0a0800 !important; text-decoration:none !important; border-radius:9px;
...
.score-before { background:linear-gradient(135deg,var(--muted),#9a8a5a); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
...
.chip { display:inline-block; background:#c9960c18; border:1px solid #c9960c33; color:var(--accent2); border-radius:100px; padding:3px 10px; font-size:11px; margin:2px; }
...
.stTabs [aria-selected="true"] { background:var(--accent) !important; color:#0a0800 !important; }
</style>
...
if go:
    if not st.session_state.resume_text:
        ...
        st.session_state.ats_after = max(int(data.get('match_score', 75)), st.session_state.ats_before + 5)
...
if st.session_state.step >= 2 and st.session_state.ai_data:
    ...
    # ── Mock Interview Card ──
    st.markdown("""
    <div class="card" style="border-color:#f59e0b44;margin-top:.5rem;background:linear-gradient(135deg,#1a1a2e,#16213e)">
        ...
        <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-bottom:.6rem">
            <span style="background:#f59e0b22;color:#f59e0b;padding:.2rem .7rem;border-radius:20px;font-size:11px;font-weight:600">₹299 / session</span>
    ...
    if st.button("🔔 Notify Me When Live", key="mock_interview_notify", use_container_width=True):
        log_payment_interest("Mock Interview ₹299", st.session_state.email or "")
        st.success("✅ You're on the list! We'll notify you as soon as mock interviews go live.")
...
<div style="text-align:center;padding:2.5rem 0 1rem;color:var(--muted);font-size:12px">
    ...
    <div style="font-size:11px">© 2026 Cvnixo. All rights reserved.</div>
</div>
""", unsafe_allow_html=True)
