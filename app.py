...
if go:
    if not st.session_state.resume_text:
        ...
        ai_score = min(int(data.get('match_score', 75)), 99)  # cap AI score at 99
        before   = st.session_state.ats_before
        # After must be: higher than before, at least before+5, never above 99
        st.session_state.ats_after = min(max(ai_score, before + 5), 99)
...
<div style="text-align:center;padding:1.5rem 0 0.5rem">
    <div style="font-family:'Syne',sans-serif;font-weight:700;font-size:1rem;color:var(--text);margin-bottom:.4rem">
        ⭐ Share Your Experience
    </div>
    <div style="color:var(--muted);font-size:13px;margin-bottom:1rem">
        Helped you land an interview? We would love to hear your story!
    </div>
</div>
""", unsafe_allow_html=True)

with st.expander("✍️ Leave a Review", expanded=False):
    reviewer_name = st.text_input("Your Name (or leave blank for anonymous)", placeholder="e.g. Rahul S.", key="reviewer_name")
    reviewer_role = st.text_input("Job Role you applied for", placeholder="e.g. Software Engineer at Google", key="reviewer_role")
    review_text = st.text_area("Your experience with ResumeReflect", placeholder="How did it help you? Did you get an interview? Any feedback?", height=120, key="review_text")
    review_rating = st.select_slider("Rating", options=["⭐", "⭐⭐", "⭐⭐⭐", "⭐⭐⭐⭐", "⭐⭐⭐⭐⭐"], value="⭐⭐⭐⭐⭐", key="review_rating")

    if st.button("Submit Review ✅", key="submit_review", use_container_width=True):
        if review_text and len(review_text.strip()) > 10:
            try:
                ...
                pass
            st.success("🙏 Thank you for your review! It means a lot to us.")
            st.balloons()
        else:
